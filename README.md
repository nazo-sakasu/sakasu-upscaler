...
通常は「**高品質補間**」を推奨します。
高品質補間は処理が軽く、文字・ロゴ・ピクト・図版でも形を崩しにくいため、普段使いに向いています。複数枚の一括処理にも適しています。

AI超解像は写真・生成AI画像・質感のある小さめ画像に向いています。
ただしWASM環境では処理が重く、大きい画像や複数枚処理ではブラウザが停止する可能性があります。
大きい画像（長辺1200px以上）や複数枚処理ではAI超解像を避け、高品質補間の使用を推奨します。

複数枚をまとめて処理したい場合は、「高品質補間」または「輪郭くっきり補正」を推奨します。
...
| モード | おすすめ用途 | 特徴 |
|--------|-------------|------|
| AI超解像 | 写真、生成AI画像、水彩、質感のあるイラスト | 細部を補完しやすいが処理は重い（WASM環境では特に遅い）|
| 高品質補間 | 文字、ロゴ、QR、ピクト、図版、フラットイラスト | 形を崩しにくく高速 |
| 輪郭くっきり補正 | 拡大せずにぼやけを整えたい画像、スキャン画像 | サイズ変更なしでシャープ化 |

**AI超解像は常に最適ではありません。**
写真や質感のある画像には有効ですが、文字・ロゴ・QRコード・ピクトなどでは形が変わる可能性があります。その場合は高品質補間を推奨します。

**高品質補間を選択した場合は、AIモデルが読み込まれていてもONNX推論は行われません。** 用途に応じて明示的にモードを選択してください。

## PDF処理について

PDF処理は補助機能です。通常はPNG / JPG / WebPなどの単体画像の高画質化を想定しています。

- PDFをアップロードすると、PDFオプションが自動的に展開されます
- 初期状態では**1ページ目のみ**処理します
- 全ページを処理したい場合は「PDF: 全ページを処理する」をONにしてください
- 複数ページを1つのPDFとして出力したい場合は「PDF: 結合PDFで出力する」をONにしてください
- PDFは一度画像化してから処理するため、文字情報・ベクター情報は保持されません
- 出力PDFは画像PDFになります
